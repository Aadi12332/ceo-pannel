import { RefreshCcw, Eye, Play, Trash2 } from "lucide-react";
...
export default function CredentialStatusAndWebhookQueues() {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          {credentials.map((c, i) => (
            <div key={i} className="border-b pb-6 last:border-b-0">
              <div className="flex items-center justify-between mb-2">
                <p className="font-medium text-[#0E1E38]">{c.name}</p>
                <span
              ...
              <div className="flex items-center justify-between text-sm flex-wrap">
                <span className="text-gray-500">Last used: {c.lastUsed}</span>

                {c.failures && (
                  <span className="text-[#CF2027]">{c.failures}</span>
                )}
      ...
      <div className="bg-white lg:rounded-xl rounded-lg lg:p-6 p-3">
        <h2 className="text-lg font-semibold">Webhook Queues</h2>
        <p className="text-sm text-[#CF2027] mb-6">3 Pending, 2 Dead</p>
          ...
          <div className="space-y-4">
            {webhookData.dead.map((w, i) => (
              <div key={i} className="border-b pb-4 last:border-b-0">
                ...
                <p className="text-sm text-gray-500">{w.time}</p>
                <p className="text-sm text-gray-400 mt-1">{w.note}</p>
                <span className="inline-block mt-2 px-2 py-0.5 text-xs rounded bg-[#FFDDDB] text-[#CF2027]">
                  {w.error}
                </span>
              </div>
